...
```bash
test-waf-auto-gen$ sam build --use-container
```
...
```bash
test-waf-auto-gen$ sam logs -n StockCheckerFunction --stack-name "test-waf-auto-gen" --tail
```
...
```bash
test-waf-auto-gen$ pip install -r tests/requirements.txt --user
# unit test
test-waf-auto-gen$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
test-waf-auto-gen$ AWS_SAM_STACK_NAME="test-waf-auto-gen" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "test-waf-auto-gen"
```
